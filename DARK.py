...
def login():
    ...
    bln = sekarang.strftime('%m')
    thn = sekarang.strftime('%Y')
    tanggal = thn + bln + tgl
    exp = expired[2] + expired[1] + expired[0]
    if tanggal >= exp:
        print (('\n').join(['' + logo + '']))
        print (('\n').join(['\033[1;97mScript Ini Sudah Kedaluarsa !!!\n']))
        sys.exit()
    else:
    	log_token()
...
def log_token():
    os.system("clear")
    peler()
    print("\033[1;97mKedaluarsa : \033[1;91m01-30-2021\033[1;97m")
    toket = input(k+"\n["+p+"×"+k+"]"+p+" Token  : ")
    try:
...
class crack:
	...
	def main(self,fl):
		try:
			...
			self.ko+=1
			print("\r\033[1;97m[ CRACK ]\033[1;97m %s/%s \033[1;92mOK \033[1;97m: %s \033[1;93mCP \033[1;97m: %s\033[1;97m"%(self.ko,len(self.fl),len(self.ada),len(self.cp)), end=' ');sys.stdout.flush()
		except:
			self.main(fl)
